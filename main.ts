...
// Database operations
ipcMain.handle('db:query', async (_event: IpcMainInvokeEvent, sql: string, params?: unknown[]) => {
  try {
    if (!dbInitialized) {
...
ipcMain.handle('db:execute', async (_event: IpcMainInvokeEvent, sql: string, params?: unknown[]) => {
  try {
    if (!dbInitialized) {
      throw new Error('Database not initialized');
    }
...
// File dialog
ipcMain.handle('file:select', async (_event: IpcMainInvokeEvent, options?: Record<string, any>) => {
...
// Directory dialog
ipcMain.handle('dir:select', async (_event: IpcMainInvokeEvent) => {
  const result = await dialog.showOpenDialog(mainWindow!, {
...
// Backup
ipcMain.handle('backup:create', async (_event: IpcMainInvokeEvent, targetDir: string) => {
  try {
